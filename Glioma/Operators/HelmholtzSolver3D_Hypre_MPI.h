...
class HelmholtzSolver3D_Hypre_MPI
{
    ...
    {
        // 0) Set up class variables + some info
        GridsizeX    = blocksPerDimension * B::sizeX;
        GridsizeY    = blocksPerDimension * B::sizeY;
        GridsizeZ    = blocksPerDimension * B::sizeZ;
        nMRAGblocks  = blocksPerDimension * blocksPerDimension * blocksPerDimension;
        
        if(bVerbose) printf("----------------------------------------------------------------------");
        if(bVerbose) printf("Hello from Helmholtz-Hypre solver \n");
        if(bVerbose) printf("Assume: uniform grid, which fits in the memory of single processor \n");
        if(bVerbose) printf("Number of MRAG blocks is multiple of number of MPI processes.    \n");
        if(bVerbose) printf("Using %d MRAG blocks, %d MPI processes     \n", nMRAGblocks, nprocs);
        if(bVerbose) printf("----------------------------------------------------------------------");
        ...
        blocksPerProcesor = nMRAGblocks / nprocs;
...
//        GridsizeY      = blocksPerDimension * B::sizeY;
//        GridsizeZ      = blocksPerDimension * B::sizeZ;
//        n_mrag_blocks  = blocksPerDimension * blocksPerDimension * blocksPerDimension;
//        
...
//        }
        ...
        if (bVerbose) printf("Done with hypre setup!\n"); //exit(0);
    }
