...
struct RightHandSide
{
	...
	RightHandSide(const RightHandSide&, double dt_ )
	{
	...
	inline void operator()(LabType& lab, const BlockInfo& info, BlockType& o) const
	{
		...
		for(int iz=0; iz<BlockType::sizeZ; iz++)
			for(int iy=0; iy<BlockType::sizeY; iy++)
				for(int ix=0; ix<BlockType::sizeX; ix++)
				{
					o(ix, iy, iz).domegadt =  10.;
					
				}
	}
};

struct RHSUpwind1rdOrder
{
	int stencil_start[3];
	int stencil_end[3];
	double dt;
	
	RHSUpwind1rdOrder(double dt_)
	{
		stencil_start[0] = stencil_start[1] = stencil_start[2] = -1;
		stencil_end[0] = stencil_end[1] = stencil_end[2] = +2;
		dt		= dt_;
	...
	RHSUpwind1rdOrder(const RHSUpwind1rdOrder&, double dt_ )
	{
		stencil_start[0] = stencil_start[1] = stencil_start[2] = -1;
		stencil_end[0] = stencil_end[1] = stencil_end[2] = +2;
		dt		= dt_;
...
struct RHSUpwind3rdOrder
{
	...
	RHSUpwind3rdOrder(const RHSUpwind3rdOrder&, double dt_ )
	{
		stencil_start[0] = stencil_start[1] = stencil_start[2] = -2;
		stencil_end[0] = stencil_end[1] = stencil_end[2] = +3;
		dt		= dt_;
